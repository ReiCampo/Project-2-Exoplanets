...
def plotter(result, x,y,z, pos3,proj=3, tarr=[]):
    x1=result[0:,0]
    y1=result[0:,1]
    z1=result[0:,2]
    ...
    if proj==0: #plots initial positions
        ...
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_title('Initial Positions, initial position of body 3=%.1f'%(pos3))
        if x==1:
    ...
    if proj==2: #creates a 2D plot
        ...
        if x==1:
            ax.plot(x1,y1, 'r', label="Star 1", markersize=10)
        if y==1:
            ax.plot(x2,y2, 'k', label="Planet")
        if z==1:
            ax.plot(x3,y3, 'b',label="Star 2", markersize=10)
    ...
    if proj==3: #creates a 3D plot
        ...
        ax.set_ylabel('y')
        ax.set_zlabel('z')
        ax.set_title('Motion of Bodies, initial position of body 3=%.1f'%(pos3))
        if x==1:
    ...
    if proj==4: #shows a velocity component of 1 body versus time
        ...
        ax.set_title('Velocity of Planet, initial position of body 3=%.1f'%(pos3))
        if x==1:
