...
def plotter(result, x,y,z, proj=3, tarr=[]):
    x1=result[0:,0]
    y1=result[0:,1]
    z1=result[0:,2]
    ...
    if proj==0: #plots initial positions
        ...
        if x==1:
            ax.plot(x1[0],y1[0], 'ro', label="Body 1", markersize=10)
        if y==1:
            ax.plot(x2[0],y2[0], 'ko', label="Body 2")
        if z==1:
            ax.plot(x3[0],y3[0], 'bo',label="Body 3", markersize=10)
    ...
    if proj==2: #creates a 2D plot
        ...
        if x==1:
            ax.plot(x1,y1, 'r', label="Body 1", markersize=10)
        if y==1:
            ax.plot(x2,y2, 'k', label="Body 2")
        if z==1:
            ax.plot(x3,y3, 'b',label="Body 3", markersize=10)
    ...
    if proj==4: #shows a velocity component of 1 body versus time
        ...
        if x==1:
            ax.plot(tarr, vx2, 'k', label="Body 1")
